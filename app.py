...
def read_card(concern: str, card: str, orientation: str, api_key: str) -> str:
    ...
    prompt = (
        ...
        "번호나 굵은 제목 라벨은 절대 쓰지 말고, 아래 다섯 흐름을 캐릭터가 자연스럽게 말하듯 풀어주세요. "
        "각 흐름은 하나의 짧은 문단(1~2문장)으로 쓰되, 문단마다 타로술사가 말을 거는 표현으로 시작하세요. "
        "문단과 문단 사이는 반드시 빈 줄로 띄웁니다. "
        "일반론이 아니라 이 고민에 맞춘 구체적인 메시지를 주세요.\n\n"
        "다섯 흐름 (순서대로, 라벨 없이 자연스럽게 이어서):\n"
        "- 지금 사용자가 처한 상황\n"
        "- 그 밑에 깔린 마음\n"
        "- 이 카드의 의미 (정방향/역방향 포함)\n"
        "- 카드와 사용자의 연결\n"
